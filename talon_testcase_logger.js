...
/* ============================================================
   TLog + 自動計装 オールインワン v2.3
    目的:
      ...
      ① 画面ロード・UI操作・バックエンド呼び出し・JS例外を一元的に記録
      ② html2canvas を用いた操作前後のスクリーンショット自動取得
      ③ console.log / warn / error / info / debug のキャプチャとサーバ送信
      を実現する。
    使い方:
      各画面の初期化コードで以下を呼び出す
        TLog.screenLoad('SCREEN_ID', 'SCREEN_NAME');
        TLogAutoInstrument.init('SCREEN_ID', { screenMode: 'auth' });
   変更履歴:
     ...
     v2.2 - [追加] TLogAutoInstrument に console キャプチャ機能を追加
            ...
            [追加] TLogAutoInstrument に console キャプチャ機能を追加
