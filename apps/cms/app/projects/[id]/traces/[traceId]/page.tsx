...
const EVENT_ICONS: Record<string, string> = {
  TRACE_START:"▶", TRACE_END:"⏹", SCREEN_LOAD:"🖥", UI_CLICK:"👆",
  ERROR:"⚠️", BACKEND:"⚙️", CONSOLE:"📟", TEST:"🧪",
};

function statusColor(s: string) {
  if (s === "COMPLETED" || s === "CLOSED") return "bg-green-900 text-green-300";
      ...
      {trace && (
        <div className={`border-b px-6 py-3 flex items-center justify-between ${dark ? "bg-slate-950 border-slate-800" : "bg-slate-50 border-slate-200"}`}>
          <div className="flex items-center gap-4 text-sm">
            ...
            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColor(trace.status)}`}>{trace.status}</span>
      ...
      {loading ? (
        ...
        <div className="flex h-[calc(100vh-120px)]">
                ...
                {logs.map((log) => (
                  <button key={log.id} onClick={() => setSelectedLog(log)}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {log.screenName && <div className={`text-xs mt-0.5 ${sub} truncate`}>{log.screenName}</div>}
                      {log.elementId && <div className="text-xs text-slate-500 truncate">#{log.elementId}</div>}
                    </div>
          ...
          <div className="w-1/2 overflow-y-auto">
            ...
            {!selectedLog ? (
              <div className="text-center py-12 text-slate-500 text-sm">タイムラインのイベントをクリックしてください</div>
            ) : (
                ...
                {selectedLog.screenshotPath && (
                  <div>
                    ...
                    <img
                      ...
                      alt="スクリーンショット" className="rounded-lg max-w-full border"
