...
function initFlowPage(fid){
  ...
  function uturnSvg(isRtl, W, COLS, rowIdx){
    // BOX=120px Arrow=60px SLOT=180px
    // LTR右端BOX中心: (COLS-1)*180 + 60
    // RTL左端BOX中心: 60
    var H=44, sw=3, col='#475569', arr=7;
    var xc = isRtl ? 60 : (COLS-1)*180 + 60;
    // marker IDをrow毎にユニーク化（同一IDが複数あるとmarkerが描画されない）
    var mid = 'mu'+rowIdx;
    var mk = '<defs>'
      + '<marker id="'+mid+'" markerWidth="'+arr+'" markerHeight="'+arr
    ...
    // SVG幅はcanvas全幅(W)にする
    var svg = '<svg width="'+W+'" height="'+H+'" style="display:block;" xmlns="http://www.w3.org/2000/svg">'
      + mk + path + '</svg>';
    return '<div class="flow-uturn" style="width:'+W+'px;">'+svg+'</div>';
  }
  if(!canvas._ev){
    canvas._ev=true;
  ...
  for(var row=0;row*COLS<seqs.length;row++){
    ...
    if(!isLast) html2+=uturnSvg(isRtl,totalW,COLS,row);
  }
