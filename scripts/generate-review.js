...
function initFlowPage(fid){
  ...
  function uturnSvg(isRtl, W, COLS){
    // 実際のrow幅 = COLS*120 + (COLS-1)*60 = COLS*180 - 60
    // LTR右端BOX中心X = (COLS-1)*180 + 60
    // RTL左端BOX中心X = 60
    var H=44, sw=3, col='#475569', arr=7;
    var rowW = COLS*180 - 60;
    ...
    var mk = '<defs>'
      ...
      + '</marker>'
      + '</defs>';
    var d = 'M'+xc+' 0 L'+xc+' '+(H-arr);
    var path = '<path d="'+d+'" stroke="'+col+'" stroke-width="'+sw
      +'" fill="none" marker-end="url(#mua)" />';
    var svg = '<svg width="'+rowW+'" height="'+H+'" xmlns="http://www.w3.org/2000/svg">'
      + mk + path + '</svg>';
    return '<div class="flow-uturn" style="width:'+rowW+'px;">'+svg+'</div>';
  }
  if(!canvas._ev){
    canvas._ev=true;
  ...
  for(var row=0;row*COLS<seqs.length;row++){
    ...
    if(!isLast) html2+=uturnSvg(isRtl,totalW,COLS);
  }
