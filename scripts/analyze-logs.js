...
/* =============================================================================
   ...
   ログ駆動型 問題・課題 自動抽出スクリプト v1.0
   Claude API 不使用 / ルールベース完全自動

   処理フロー:
     1. logs/features/*.jsonl を全件読み込み
     2. 10種の検出ルールで問題候補を抽出
     3. 優先度スコアを自動算出（重大度×再現性×頻度×信頼度）
     4. docs/issues/issues.json に出力

   検出ルール一覧:
     R01 - ERRORログ検出
     R02 - UNKNOWN featureId（ロガー初期化ミス）
     R03 - ボタン重複バインド（同一ボタンが10ms以内に複数回記録）
     R04 - 検索後の画面未更新（SEARCH_RESULT = 取得不可）
     R05 - ボタン押下後BACKEND無応答（API未呼び出し）
     R06 - BACK_BUTTON連打（短時間内に3回以上）
     R07 - SCREEN_LOAD重複（1秒以内に複数回）
     R08 - BACKEND失敗（status !== SUCCESS）
     R09 - クリア後もフォーム値残存
     R10 - ログ量過少（操作が記録されていない）
   =============================================================================*/
...
function calcPriorityScore(severity, reproducibility, frequency, confidence) {
  ...
  const fw = frequency;   // 0.0〜1.0
  const cw = confidence;  // 0.0〜1.0
  return Math.round((sw * 40) + (rw * 20) + (fw * 20) + (cw * 20));
}
...
function detectR05_ApiNotCalled(featureId, logs) {
  ...
  actionClicks.forEach(click => {
    ...
    const backend = logs.find(e =>
      e.type === 'BACKEND' &&
      e.ts > click.ts &&
      timeDiffMs(click.ts, e.ts) <= afterMs &&
